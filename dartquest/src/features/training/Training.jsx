...
function ConfigSheet({ task, onCancel, onSave }) {
  ...
  return <div className="training-sheet-backdrop" role="presentation" onClick={onCancel}><section className="training-sheet" role="dialog" aria-modal="true" aria-labelledby="training-edit-title" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="training-config-fields">
      {'hitsPerTarget' in configuration && <label>Treffer pro Feld<input type="number" min="1" max="20" value={configuration.hitsPerTarget} onChange={(e) => update('hitsPerTarget', e.target.value)} /></label>}
...
function TrainingGameplay({ tasks, players, activeProfile, onExit }) {
  ...
  return <main className="training-game" style={{'--training-player-color':player.color || '#42e695'}}><header><div><span>TRAINING · {player.name}</span><strong>AUFGABE {progress.currentTaskIndex+1} / {tasks.length}</strong></div><button type="button" onClick={onExit}>BEENDEN</button></header><section className="training-game-main" data-progression={progressionTask || undefined} data-field-complete={progressionTask && taskState.targetIndex > 0 || undefined}><h1>{task.title}</h1><p aria-live={progressionTask ? "polite" : undefined}>{progressionTask ? `Ziel: ${target} / Fortschritt: ${taskState.hitsOnTarget}/${task.configuration.hitsPerTarget}` : target ? `ZIEL: ${target}` : describeTrainingTask(task)}</p>
    ...
    <div className="training-status"><div><span>{task.type === 'checkout' ? 'CHECKOUT' : 'AUFNAHME'}</span><strong>{currentVisitNumber}{totalVisits != null && ` / ${totalVisits}`}</strong></div><div><span>PUNKTE</span><strong>{taskState.score} / {getTrainingTaskMaxScore(task)}</strong></div></div>
    ...
    {task.type === 'highscore' ? <ScoreKeypad value={scoreInput} onChange={setScoreInput} onConfirm={() => { apply({result:'score',value:Number(scoreInput)}); setScoreInput('') }} disabled={false} fill /> : task.type === 'checkout' ? <div className="training-checkout-buttons">{checkoutChoices.map((darts) => <button type="button" key={darts} onClick={() => apply({result:'checkout',darts})}>{darts} {darts===1?'DART':'DARTS'}<small>+{task.configuration.scoring[darts] ?? 0}</small></button>)}<button type="button" onClick={() => apply({result:'miss'})}>NICHT GESCHAFFT<small>+0</small></button></div> : <div className="training-hit-buttons">
      {(task.type === 'bull' ? [['bull','OUTER BULL'],['bullseye','BULLSEYE']] : task.configuration.ring === 'single' ? [['single','SINGLE']] : task.configuration.ring === 'double' ? [['double','DOUBLE']] : task.configuration.ring === 'triple' ? [['triple','TRIPLE']] : [['single','SINGLE'],['double','DOUBLE'],['triple','TRIPLE']]).map(([result,label]) => <button type="button" key={result} onClick={() => apply({result})}>{label}<small>+{task.configuration.scoring[result]}</small></button>)}
