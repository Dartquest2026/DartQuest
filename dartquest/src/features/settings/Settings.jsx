...
import WhatsNew from '../releases/WhatsNew'
...
function Settings({ activeProfile, onBack, onOpenProfile, onLogout }) {
  ...
  const controlsRef = useVisibleFeature('settings-controls')
  const transferRef = useVisibleFeature('campaign-transfer')
  ...
  if (whatsNewOpen) return <WhatsNew onBack={() => setWhatsNewOpen(false)} />
  ...
  return <main className="settings-screen">
    ...
    <section ref={controlsRef} className="settings-section"><div className="settings-section-heading"><span aria-hidden="true">⚙</span><div><p>ALLGEMEIN</p><h2>Darstellung &amp; Gerät <NewBadge featureId="settings-controls" /></h2></div></div><div className="settings-list">
    ...
    <section className="settings-section"><div className="settings-section-heading"><span aria-hidden="true">i</span><div><p>TUTORIALS &amp; HINWEISE</p><h2>Vorhandene Hinweise</h2></div></div><div className="settings-list"><button ref={tutorialResetButtonRef} type="button" onClick={() => { markSeen('tutorial-reset'); requestTutorialReset() }}><span><strong>Bedienhinweise zurücksetzen <NewBadge featureId="tutorial-reset" /></strong><small>Den Eingabemodus-Hinweis in Level 1 erneut anzeigen</small></span><b>›</b></button></div></section>

    <section ref={transferRef} className="settings-section" id="settings-game-transfer"><div className="settings-section-heading"><span aria-hidden="true">↔</span><div><p>SPIELSTAND &amp; GERÄTE</p><h2>Fortschritt verwalten <NewBadge featureId="campaign-transfer" /></h2></div></div><CampaignTransfer accountId={activeProfile.id} onMessage={setMessage} /><button className="settings-danger-row" type="button" onClick={() => setResetOpen(true)}>Fortschritt zurücksetzen</button></section>
    ...
    <section className="settings-section"><div className="settings-section-heading"><span aria-hidden="true">♙</span><div><p>KONTO &amp; DATENSCHUTZ</p><h2>Account</h2></div></div><div className="settings-list">
      ...
      <button type="button" disabled={busy} onClick={() => { markSeen('password-recovery'); sendPasswordReset() }}><span><strong>Passwort ändern <NewBadge featureId="password-recovery" /></strong><small>Sicheren Reset-Link per E-Mail senden</small></span><b>›</b></button>
    ...
    <section className="settings-section"><div className="settings-section-heading"><span aria-hidden="true">?</span><div><p>HILFE</p><h2>Support</h2></div></div><div className="settings-list"><button type="button" onClick={() => { markSeen('help-contact'); setHelpOpen(true) }}><span><strong>Hilfe &amp; Kontakt <NewBadge featureId="help-contact" /></strong><small>FAQ und Kontaktmöglichkeit</small></span><b>›</b></button><button type="button" onClick={() => setWhatsNewOpen(true)}><span><strong>Was ist neu?</strong><small>Neuerungen in Version {APP_VERSION}</small></span><b>›</b></button></div></section>
