...
function BossDefeatedSequence({ confirmation, onContinue }) {
  ...
  return <section className={`boss-defeated boss-theme-${presentation.theme}`} style={getBossPresentationStyle(presentation)} role="dialog" aria-modal="true" aria-labelledby="boss-defeated-title">
    ...
    <button ref={continueButton} type="button" onClick={requestContinue}>{confirmation.newWorldName ? 'WEITER ZUR NEUEN WELT' : 'WEITER ZUR KAMPAGNE'}</button>
  </section>
}
